...
export default function CreateWentop() {
  ...
  const [status, setStatus] = useState<"ABIERTA" | "CERRADA">("ABIERTA");

  useEffect(() => {
    if (session?.user?.name && !observerName) {
      setObserverName(session.user.name);
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Append our custom selected files
    ...
    try {
      const result = await createWentop(formData);
      if (result.success) {
        alert("¡WENTOP enviada con éxito!");
        router.push("/my-wentops");
        router.refresh();
      }
    } catch (error: any) {
      alert("Error al enviar WENTOP: " + error.message);
    } finally {
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <a href="/my-wentops" style={{ color: "var(--text-secondary)", textDecoration: "none", fontSize: "1.2rem", lineHeight: 1 }}>←</a>
          <div>
            <h1 style={{ fontSize: "1.1rem", fontWeight: "800", margin: 0, letterSpacing: "-0.02em" }}>Nueva Tarjeta WENTOP</h1>
            <p style={{ fontSize: "0.75rem", color: "var(--text-secondary)", margin: 0 }}>Observación Preventiva</p>
          </div>
        </div>
        <img 
          src="/logo.jpg" 
      ...
      <form id="wentop-form" onSubmit={handleSubmit} style={{ padding: "20px 16px 120px", maxWidth: "600px", margin: "0 auto" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
          ...
          <div className="form-group">
            ...
            <select name="observedSector" className="form-select" required>
          ...
          {/* 14 & 15. Acción de cierre y Fecha de cierre (visibles/destacados si es CERRADA) */}
          {status === "CERRADA" && (
            <div style={{
              ...
              {selectedFiles.length > 0 && (
                <div style={{ marginTop: "16px", textAlign: "left", width: "100%" }}>
                  <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", marginBottom: "12px", textAlign: "center" }}>
                    {selectedFiles.length} de 5 archivos seleccionados:
                  </p>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: "600px", margin: "0 auto" }}>
          ...
          >
            {isSubmitting ? "⏳ Enviando..." : "✅ Enviar WENTOP"}
          </button>
        </div>
      </div>
    </div>
